...
fn boost_army(army: &mut Vec<Group>, boost: u32) {
    for group in army.iter_mut() {
...
fn main() {
    ...
    loop {
        ...
        boost_army(&mut immune_system_army, boost);

        while immune_system_army.len() > 0 && infection_army.len() > 0 {
            // TARGET SELECTION PHASE
            let comparison = |group_a: &Group, group_b: &Group| {
                let effective_power_order = (group_b.size * group_b.attack_damage)
                    .cmp(&(group_a.size * group_a.attack_damage));
                if effective_power_order == Ordering::Equal {
        ...
        if infection_army.is_empty() {
            let sum: u32 = immune_system_army.iter().map(|group| group.size).sum();
            println!("{}", sum);
            break;
        }
        boost += 1;
